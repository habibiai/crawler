...
def check_message(test=False):
    ...
    try:
        ...
        crawled_content = crawling(crawl_url, 'text-c2')
        # crawled_content = crawling(os.environ['CRAWL_URL'], 'text-c2')
    except:
        send_message(chatId, 'Der Crawler ist kaputt', api_key)
    ...
    if test:
        send_message(chatId, 'Das ist ein Test.', api_key)
    ...
    if no_vaccines in crawled_content:
        ...
        send_message(chatId, 'Es könnte neue Termine geben!', api_key)
...
def summary_message():
    ...
    send_message(chatId, 'Der Crawler läuft noch', api_key)
